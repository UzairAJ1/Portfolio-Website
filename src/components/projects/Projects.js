    ...
    return (
        ...
        <>
            ...
            <div className=' container mx-auto w-full h-screen flex flex-col justify-center items-center'>
                ...
                <button onClick={()=>{setScroll(scroll-1)}} className={scroll>0 ? "w-52 h-24 absolute left-24  z-20 mt-32" : "hidden"}>
                <ArrowBackIosIcon  fontSize="large"/>
                </button>
                <div className='snap-x snap-mandatory flex overflow-scroll overflow-y-hidden w-full items-center md:h-4/6 h-full scrollbar scrollbar-thumb-amber-500 scrollbar-track-[#3C3C3C]'>
                   {scroll===0 &&
                   ...
                   <ProjetcsCard backgroundImg={css} title="Css Project" description="Buildings made with CSS are an impressive example of how cascading style sheets can be used to create complex, visually stunning designs. Using a combination of positioning, transforms, gradients, and other CSS properties, it is possible to simulate the appearance of skyscrapers, towers, and other structures. CSS buildings can be a fun and creative way to showcase the power and flexibility of this popular web design technology."/>
                    ...
                    {scroll===4 &&
                   <ProjetcsCard backgroundImg={tailwind} title="Tailwind Landing Page"/>
                   }
                   
                </div>
                <button onClick={()=>{setScroll(scroll+1)}} className={scroll<4 ? "w-52 h-24 absolute right-24 z-20 mt-32":"hidden"}><ArrowForwardIosIcon fontSize="large"/></button>
            </div>
        </>
